...
view_r=12       -- how far you can SEE (line of
                -- sight); lit tiles past this
                -- stay hidden until you approach
 ...
 for c=0,mapw-1 do
  for r=0,maph-1 do
   ...
   if s==5 then
    ...
    mset(c,r,0)
   elseif s==6 then
    add(enemies,make_walker(x,y)) mset(c,r,0)
   elseif s==7 then
    add(enemies,make_jumper(x,y)) mset(c,r,0)
   elseif s==8 then
    add(enemies,make_flyer(x,y)) mset(c,r,0)
   elseif s==9 then
...
function compute_light(cc,cr)
 -- window covers the screen + a view-radius
 -- margin (so light/sight just off-screen
 -- still counts)
 local m=view_r+1
 lgc0=cc-m lgr0=cr-m
 lgw=16+m*2 lgh=16+m*2
 lit={}   -- floor light levels (all sources)
 seen={}  -- floors the PLAYER has line-of-sight to
 -- player: marks what's SEEN (out to view_r)
 -- and lights floors out to light_r
 if p.is_light then
  flood(flr((p.x+pw/2)/8),flr((p.y+ph/2)/8),view_r,light_r,true)
 end
 -- torches: light floors only (don't grant sight)
...
-- never entered). marks seen (if ms) out to
-- rng, and lights floors out to lr. walls are
-- NOT lit here -- they take their brightness
-- from visible floors at draw time, so a torch
-- behind a wall can't bleed through it.
...
function flood(sc,sr,rng,lr,ms)
 ...
 while h<=ln do
  ...
  if ms then set_seen(c,r) end
  if d<lr then set_lit(c,r,lr-d) end
  if d<rng then
   for nb in all(nbrs) do
    local nc,nr=c+nb[1],r+nb[2]
    if not is_opaque(nc,nr) then
     local ok=true
     if nb[1]!=0 and nb[2]!=0 then
      -- no squeezing through a diagonal crack
      ok=(not is_opaque(c+nb[1],r))
       or (not is_opaque(c,r+nb[2]))
     end
     if ok then push_l(nc,nr,d+1) end
    end
   end
  end
 end
...
function push_l(c,r,d)
 if c<lgc0 or r<lgr0
  or c>=lgc0+lgw or r>=lgr0+lgh then return end
 local k=c+r*1024
 if lvis[k] then return end
 lvis[k]=true
 ln+=1 lqc[ln]=c lqr[ln]=r lqd[ln]=d
end

function set_lit(c,r,lv)
 if lv<=0 then return end
 if c<lgc0 or r<lgr0
  or c>=lgc0+lgw or r>=lgr0+lgh then return end
 local i=(r-lgr0)*lgw+(c-lgc0)
 if (lit[i] or 0)<lv then lit[i]=lv end
end

function get_lit(c,r)
 if c<lgc0 or r<lgr0
  or c>=lgc0+lgw or r>=lgr0+lgh then return 0 end
 return lit[(r-lgr0)*lgw+(c-lgc0)] or 0
end

function set_seen(c,r)
 seen[(r-lgr0)*lgw+(c-lgc0)]=true
end

function seen_at(c,r)
 if c<lgc0 or r<lgr0
  or c>=lgc0+lgw or r>=lgr0+lgh then return false end
 return seen[(r-lgr0)*lgw+(c-lgc0)]
end
...
function draw_hud()
 ...
 -- hearts
 for i=1,player_maxhp do
  draw_heart(44+(i-1)*7,2,i<=p.hp)
 end

 -- boss hp bar (bottom, during the fight)
 if boss and boss.alive then
...

...
__map__

...

